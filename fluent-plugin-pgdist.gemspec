# Generated by jeweler
# DO NOT EDIT THIS FILE DIRECTLY
# Instead, edit Jeweler::Tasks in Rakefile, and run 'rake gemspec'
# -*- encoding: utf-8 -*-

Gem::Specification.new do |s|
  s.name = "fluent-plugin-pgdist"
  s.version = "0.2.0"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.authors = ["Kenji Hara"]
  s.date = "2013-05-02"
  s.description = "Fluentd plugin for distribute insert into PostgreSQL"
  s.email = "haracane@gmail.com"
  s.extra_rdoc_files = [
    "LICENSE.txt",
    "README.md"
  ]
  s.files = [
    ".document",
    ".rspec",
    "Gemfile",
    "LICENSE.txt",
    "README.md",
    "Rakefile",
    "VERSION",
    "fluent-plugin-pgdist.gemspec",
    "lib/fluent/plugin/out_pgdist.rb",
    "spec/lib/fluent/plugin/out_pgdist_spec.rb",
    "spec/spec_helper.rb"
  ]
  s.homepage = "http://github.com/haracane/fluent-plugin-pgdist"
  s.licenses = ["MIT"]
  s.require_paths = ["lib"]
  s.rubygems_version = "1.8.24"
  s.summary = "Fluentd plugin for distribute insert into PostgreSQL"

  if s.respond_to? :specification_version then
    s.specification_version = 3

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<fluentd>, ["~> 0.10.33"])
      s.add_runtime_dependency(%q<pg>, ["~> 0.15.1"])
      s.add_development_dependency(%q<rspec>, ["~> 2.12.0"])
      s.add_development_dependency(%q<yard>, ["~> 0.8.3"])
      s.add_development_dependency(%q<redcarpet>, ["~> 2.2.2"])
      s.add_development_dependency(%q<rdoc>, ["~> 3.12"])
      s.add_development_dependency(%q<bundler>, [">= 0"])
      s.add_development_dependency(%q<jeweler>, ["~> 1.8.4"])
      s.add_development_dependency(%q<simplecov>, ["~> 0.7.1"])
      s.add_development_dependency(%q<simplecov-rcov>, ["~> 0.2.3"])
      s.add_development_dependency(%q<ci_reporter>, ["~> 1.8.3"])
      s.add_development_dependency(%q<flog>, ["~> 3.2.1"])
    else
      s.add_dependency(%q<fluentd>, ["~> 0.10.33"])
      s.add_dependency(%q<pg>, ["~> 0.15.1"])
      s.add_dependency(%q<rspec>, ["~> 2.12.0"])
      s.add_dependency(%q<yard>, ["~> 0.8.3"])
      s.add_dependency(%q<redcarpet>, ["~> 2.2.2"])
      s.add_dependency(%q<rdoc>, ["~> 3.12"])
      s.add_dependency(%q<bundler>, [">= 0"])
      s.add_dependency(%q<jeweler>, ["~> 1.8.4"])
      s.add_dependency(%q<simplecov>, ["~> 0.7.1"])
      s.add_dependency(%q<simplecov-rcov>, ["~> 0.2.3"])
      s.add_dependency(%q<ci_reporter>, ["~> 1.8.3"])
      s.add_dependency(%q<flog>, ["~> 3.2.1"])
    end
  else
    s.add_dependency(%q<fluentd>, ["~> 0.10.33"])
    s.add_dependency(%q<pg>, ["~> 0.15.1"])
    s.add_dependency(%q<rspec>, ["~> 2.12.0"])
    s.add_dependency(%q<yard>, ["~> 0.8.3"])
    s.add_dependency(%q<redcarpet>, ["~> 2.2.2"])
    s.add_dependency(%q<rdoc>, ["~> 3.12"])
    s.add_dependency(%q<bundler>, [">= 0"])
    s.add_dependency(%q<jeweler>, ["~> 1.8.4"])
    s.add_dependency(%q<simplecov>, ["~> 0.7.1"])
    s.add_dependency(%q<simplecov-rcov>, ["~> 0.2.3"])
    s.add_dependency(%q<ci_reporter>, ["~> 1.8.3"])
    s.add_dependency(%q<flog>, ["~> 3.2.1"])
  end
end

